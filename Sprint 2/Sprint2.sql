...
JOIN transaction
ON company.id = transaction.company_id
WHERE declined = 0 #declined = 0 para comprobar que al menos tengan una compra sin rechazar
...
SELECT DISTINCT company_name AS Empresas FROM company
WHERE id IN(
...
SELECT c.company_name AS Empresa, amount AS Cantidad, c.country AS País, t.id AS ID_Transacción
FROM company c
JOIN transaction t
ON c.id = t.company_id
WHERE c.country = (
...
SELECT c.company_name AS Empresa, c.phone AS Teléfono, c.country AS País, DATE(t.timestamp) AS Fecha, t.amount
FROM company c
JOIN transaction t
ON c.id = t.company_id
...
SELECT c.company_name as Nombre, COUNT(t.amount) as Transacciones,
	CASE WHEN COUNT(t.amount) > 4 THEN 'Más de 4.'
    ELSE 'Menos de 4.' END AS Cantidad
FROM transaction t
JOIN company c
ON c.id = t.company_id
GROUP BY Nombre
ORDER BY Cantidad, Nombre ASC;